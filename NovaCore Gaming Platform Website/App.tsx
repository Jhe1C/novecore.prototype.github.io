...
function Breadcrumb({
  ...
  if (
    !metadata ||
    navigationState.currentPage === PageType.HOME
  )
    return null;
  ...
  const handleBreadcrumbClick = (index: number) => {
    ...
    if (index === 0) {
      ...
      index === 1 &&
      metadata.breadcrumb[1] === "Browse Games"
    ) {
      onNavigate(PageType.BROWSE);
    } else if (
      index === 1 &&
      metadata.breadcrumb[1] === "NovaVerse"
    ) {
      onNavigate(PageType.NOVAVERSE);
    } else if (
      index === 1 &&
      metadata.breadcrumb[1] === "My Profile"
    ) {
      onNavigate(PageType.PROFILE);
    } else if (
      index === 1 &&
      metadata.breadcrumb[1] === "Shopping Cart"
    ) {
  ...
  return (
    <nav
      className="container mx-auto px-6 py-4 border-b border-[var(--border-subtle)]"
        ...
        {metadata.breadcrumb.map((crumb, index) => (
          <li key={index} className="flex items-center">
            ...
            {index === metadata.breadcrumb.length - 1 ? (
              ...
              <button
                className="text-[var(--text-secondary)] hover:text-[var(--accent-blue)] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[var(--accent-blue)] focus:ring-opacity-50 rounded"
      ...
      {navigationState.canGoBack && (
        <div className="mt-3">
          <Button
            ...
            onClick={() =>
              onNavigate(navigationState.previousPage!)
            }
          >
...
function LoadingSpinner({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 space-y-4">
      ...
      <p
        className="text-[var(--text-secondary)] text-center max-w-sm"
        role="status"
        aria-live="polite"
      >
        {message}
      ...
      {progress !== undefined && (
        <div className="w-48 h-2 bg-[var(--slate-medium)] rounded-full overflow-hidden">
          <div
...
function ErrorFallback({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <Card className="vanguard-card p-8 max-w-lg text-center">
        <div className="w-16 h-16 bg-[var(--destructive)]/10 rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <div className="space-y-3 mb-6">
          {errorAdvice.actions.map((action, index) => (
            <Button
              ...
              className={
                action.primary
                  ? "vanguard-btn-primary w-full"
                  : "vanguard-btn-secondary w-full"
              }
        ...
        <details className="mt-6 text-left">
          <summary className="text-sm text-[var(--text-muted)] cursor-pointer hover:text-[var(--accent-blue)]">
            錯誤詳細資訊
          </summary>
          <pre className="text-xs text-[var(--text-muted)] mt-2 p-3 bg-[var(--slate-medium)] rounded overflow-auto">
            {error.message}
            {error.stack && "\n\n" + error.stack}
          </pre>
        </details>
...
function ConfirmationDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="vanguard-card border-[var(--border-subtle)]">
        <DialogHeader>
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bg-[var(--destructive)] text-white py-2 px-4 text-center z-50">
      <div className="flex items-center justify-center space-x-2">
        ...
        <span className="text-sm">您目前處於離線狀態</span>
      </div>
    </div>
  );
}
...
function PlaceholderPage({
  ...
  return (
    <div className="container mx-auto px-6 py-16 content-width text-center">
      <div className="w-24 h-24 bg-gradient-to-br from-[var(--accent-blue)] to-[var(--accent-purple)] rounded-full flex items-center justify-center mx-auto mb-8 cosmic-glow">
        <Icon className="w-12 h-12 text-white" />
      </div>
      <h1 className="text-4xl mb-4 text-gradient">{title}</h1>
...
export default function App() {
    ...
    (
      ...
      setAppState((prev) => {
        const newHistory = replaceHistory
          ? [
              ...prev.navigation.navigationHistory.slice(0, -1),
              newPage,
            ]
        ...
        return {
          ...
          navigation: {
            ...
            selectedGameId:
              gameId ||
              (newPage === PageType.GAME_PRODUCT
                ? prev.navigation.selectedGameId
                : null),
  ...
  const handleGameSelect = useCallback(
    (game: GameData) => {
      handleNavigate(PageType.GAME_PRODUCT, {
        gameId: game.id,
      });
  ...
  const handleAddToCart = useCallback(
    (game: GameData, edition?: any) => {
      ...
      toast.success(
        `已將${game.titleCN || game.title}加入購物車！`,
        {
  ...
  const handleRemoveFromCart = useCallback((itemId: string) => {
    setAppState((prev) => {
      const removedItem = prev.cart.items.find(
        (item) => item.id === itemId,
      );
  ...
  useEffect(() => {
    const navigationEventHandlers = {
      ...
      "navigate-to-free-zone": () =>
        handleNavigate(PageType.FREE_ZONE),
    };

    Object.entries(navigationEventHandlers).forEach(
      ([event, handler]) => {
  ...
  useEffect(() => {
    ...
    appState.ui.showConfirmDialog,
    appState.navigation.canGoBack,
    appState.navigation.previousPage,
    handleNavigate,
  ]);
        ...
        case PageType.HOME:
          return (
            <>
              ...
              <NovaVerseTeaser
                onNavigate={(page) =>
                  handleNavigate(page as PageType)
                }
              />
            </>
          );
          ...
          return (
            <PlaceholderPage
              ...
              actions={[
                {
                  label: "聯繫客服",
                  onClick: () => toast.info("客服系統即將推出"),
                },
                {
                  label: "常見問題",
                  onClick: () => toast.info("FAQ頁面開發中"),
                  variant: "outline",
                },
              ]}
            />
          ...
          return (
            <PlaceholderPage
              ...
              actions={[
                {
                  label: "下載啟動器",
                  onClick: () => toast.info("啟動器即將推出"),
                },
                {
                  label: "系統需求",
                  onClick: () =>
                    toast.info("系統需求頁面開發中"),
                  variant: "outline",
                },
              ]}
            />
    ...
    <WishlistProvider>
      <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] digital-vanguard-bg">
        ...
        <ConfirmationDialog
          ...
          variant={
            appState.ui.confirmMessage.includes("清空")
              ? "destructive"
              : "default"
          }
        />
        ...
        <footer className="border-t border-[var(--border-subtle)] mt-20">
          <div className="container mx-auto px-6 py-12 content-width">
            <div className="grid md:grid-cols-4 gap-8">
              <div className="md:col-span-2">
                ...
                <div className="flex space-x-4">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="vanguard-btn-ghost w-10 h-10 p-0"
                    aria-label="GitHub"
                  >
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-[var(--text-secondary)]">
                  <li>
                    <button
                      className="hover:text-[var(--accent-blue)] transition-colors text-left"
                      onClick={() =>
                        handleNavigate(PageType.BROWSE)
                      }
                    >
                  ...
                  <li>
                    <button
                      className="hover:text-[var(--accent-blue)] transition-colors text-left"
                      onClick={() =>
                        handleNavigate(PageType.LAUNCHER)
                      }
                    >
                      下載啟動器
                    </button>
                ...
                <ul className="space-y-2 text-sm text-[var(--text-secondary)]">
                  <li>
                    <button
                      className="hover:text-[var(--accent-blue)] transition-colors text-left"
                      onClick={() =>
                        handleNavigate(PageType.SUPPORT)
                      }
                    >
                      幫助中心
                    </button>
            ...
            <div className="border-t border-[var(--border-subtle)] mt-12 pt-8 flex flex-col md:flex-row items-center justify-between">
              <p className="text-sm text-[var(--text-muted)]">
                © 2024 NovaCore.
                版權所有。採用現代網路技術建構。
              </p>
              <div className="flex items-center space-x-6 mt-4 md:mt-0">
                <button className="text-sm text-[var(--text-muted)] hover:text-[var(--accent-blue)] transition-colors">
                  隱私政策
                </button>
                <button className="text-sm text-[var(--text-muted)] hover:text-[var(--accent-blue)] transition-colors">
                  服務條款
